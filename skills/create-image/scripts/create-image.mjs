...
function assertUrl(value, label) {
  try {
    ...
    throw new Error(`${label} must be an http(s) URL accessible by fal.ai. Local files are not uploaded by this helper.`);
  }
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--image-urls":
      case "--image_urls":
        options.image_urls.push(...parseImageUrlList(readValue(argv, i, arg), arg));
        i += 1;
        break;
      case "--mask-url":
      case "--mask_url":
      ...
      case "--quality": {
        ...
        if (quality !== "medium") throw new Error("This skill always uses medium quality");
      ...
      case "-n": {
        ...
        if (numImages !== 1) throw new Error("This skill always generates exactly 1 image");
        i += 1;
        break;
      }
      case "--format":
      ...
      case "--output_format": {
        ...
        if (format !== "png") throw new Error("This skill always outputs PNG images");
        i += 1;
        break;
      }
      case "--sync-mode":
  ...
  const hasEditInputs = options.image_urls.length > 0 || Boolean(options.mask_url);
  ...
  if (!options.image_size) options.image_size = options.mode === "edit" ? "auto" : "landscape_4_3";
...
async function downloadImage(image, index, outDir, prompt, outputFormat, timeoutMs) {
  ...
  if (typeof image.url === "string" && image.url.startsWith("data:")) {
    ...
    if (!response.ok) {
      throw new Error(`Failed to download image ${index + 1}: HTTP ${response.status} ${await response.text()}`);
    }
  ...
  return { filePath, bytes: data.length, contentType, width: image.width, height: image.height, url: image.url };
}
...
async function main() {
  ...
  try {
    ...
    throw new Error(`fal returned non-JSON response (HTTP ${response.status}): ${responseText}`);
  }

  if (!response.ok) {
    throw new Error(`fal request failed (HTTP ${response.status}): ${JSON.stringify(result, null, 2)}`);
  }

  const images = result.images || result.data?.images;
  if (!Array.isArray(images) || images.length === 0) {
    throw new Error(`fal response did not include images: ${JSON.stringify(result, null, 2)}`);
  }
    ...
    JSON.stringify(
      {
        ...
        images: images.map((image) => ({
          ...
          width: image.width,
          height: image.height,
          url: typeof image.url === "string" && image.url.startsWith("data:") ? "data:(omitted)" : image.url,
        })),
      },
      null,
      2,
  ...
  for (let i = 0; i < images.length; i += 1) {
    const saved = await downloadImage(images[i], i, outDir, input.prompt, input.output_format, timeoutMs);
    console.log(

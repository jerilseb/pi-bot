...
const GEMINI_GENERATE_CONTENT_API =
...
export async function transcribeAudio(
	...
	return {
		ok: false,
		error:
			"Set GOOGLE_GENAI_API_KEY/GEMINI_API_KEY or ELEVENLABS_API_KEY to enable transcription.",
	};
}
...
async function synthesizeWithGoogleGenAI(
	text: string,
): Promise<TtsAudioResult> {
	const requestBody = {
		contents: [
...
async function retrySpeechRequest<T>(
	...
	return {
		...
		error:
			attempt > 1 && lastError
				? `${lastError} after ${attempt} attempts`
				: lastError,
	};
}

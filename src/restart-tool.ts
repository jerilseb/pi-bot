...
export function restartToolExtension(
  chatId: string,
  ...
  return (pi: ExtensionAPI) => {
    pi.registerTool({
      ...
      label: 'Restart Bot',
      description:
        'Restart the bot process. Use only when the user explicitly asks the assistant to restart itself or restart the bot. Optionally stores a self-contained task to run automatically after PM2 brings the bot back up.',
      ...
      async execute(_toolCallId, params: RestartBotParamsType) {
        ...
        gateway.notice(chatId, '🧪 Running pre-restart checks...');

...
function appendActiveModelToSystemPrompt(systemPrompt: string): string {
  return [
    ...
    'The heartbeat/cron background model is configured only by CONFIG_BACKGROUND_MODEL in src/config.ts and cannot be changed from the web UI.',
    ...
    'When the user changes chat models with the model picker, the bot updates this file.',
  ].join('\n');
}

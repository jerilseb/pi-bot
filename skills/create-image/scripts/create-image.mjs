...
const QUERY_ENDPOINT = process.env.KIE_QUERY_ENDPOINT || new URL("/api/v1/jobs/recordInfo", BASE_URL).toString();
...
function aspectRatioFromDimensions(width, height, label) {
	...
	if (!best || best.diff > 0.03) {
		throw new Error(`${label} dimensions must match one of KIE's supported ratios: 1:1, 9:16, 16:9, 4:3, 3:4`);
	}
	return best.ratio;
}
...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i += 1) {
		...
		switch (arg) {
			...
			case "-n": {
				...
				if (numImages !== 1)
					throw new Error("This skill downloads exactly 1 image result");
				i += 1;
				break;
			}
			case "--format":
	...
	if (options.prompt.length > 20_000)
		throw new Error("KIE prompt maximum is 20,000 characters");
...
async function requestJson(url, options, timeoutMs, label) {
	...
	if (Object.hasOwn(result, "code") && Number(result.code) !== 200) {
		throw new Error(
			`${label} failed (code ${result.code}): ${JSON.stringify(result, null, 2)}`,
		);
	}
	return result;
}
...
function looksLikeImagePayload(value) {
	return typeof value === "string" && (value.startsWith("http://") || value.startsWith("https://") || value.startsWith("data:"));
}
...
function isTerminalFailure(state) {
	return ["fail", "failed", "failure", "error", "cancelled", "canceled", "timeout", "rejected"].includes(state);
}
...
async function main() {
	...
	if (!taskId) {
		throw new Error(`KIE create response did not include taskId: ${JSON.stringify(createResult, null, 2)}`);
	}
	console.log(
		"CREATE_RESPONSE",
		JSON.stringify(

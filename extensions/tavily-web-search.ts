...
function buildResultText(data: TavilyResponse, params: TavilySearchParamsType): string {
  const lines: string[] = [];
  ...
  if (results.length > 0) {
    ...
    results.forEach((result, index) => {
      lines.push(`${index + 1}. ${cleanText(result.title) || 'Untitled result'}`);
...
export default function tavilySearchExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      ...
      const resultText = buildResultText(data, params as TavilySearchParamsType);

...
const SendImageParams = Type.Object({
	...
	caption: Type.Optional(
		Type.String({
			description:
				"Optional short caption shown beneath the image in Telegram.",
		}),
	),
});
...
const SendDocumentParams = Type.Object({
	...
	caption: Type.Optional(
		Type.String({
			description:
				"Optional short caption shown beneath the document in Telegram.",
		}),
	),
});

export function telegramImageExtension(
	chatId: string,
...
export function telegramDocumentExtension(
	chatId: string,
): (pi: ExtensionAPI) => void {
	return (pi: ExtensionAPI) => {
		pi.registerTool({
...
function validateUpload(
	...
	const ok = allowedDirs.some((dir) => {
		...
		return (
			realFile === resolved || realFile.startsWith(`${resolved}${path.sep}`)
		);
	});

	if (!ok) {
		throw new Error(
			`File is not under an allowed upload directory: ${filePath}`,
		);
	}
}

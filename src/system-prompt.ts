...
function appendActiveModelToSystemPrompt(systemPrompt: string): string {
  return [
    ...
    'The scheduled heartbeat runs only when that file sets "heartbeat": true and HEARTBEAT_MODEL is set in .env; scheduled tasks run only when it sets "cronJobs": true and SCHEDULED_TASK_MODEL is set in .env. Cron jobs and scheduled tasks are the same feature; the terms are interchangeable. Both settings default to off and are read at startup, so changing either requires a restart to take effect. A feature switched on without its model is a startup error, so the bot will not run in that state.',
    ...
    'The heartbeat model is HEARTBEAT_MODEL in .env and scheduled tasks default to SCHEDULED_TASK_MODEL in .env; there is no shared fallback between them. A task can name its own model through the model parameter of create_schedule_task or update_scheduled_task. Neither default can be changed from Telegram.',

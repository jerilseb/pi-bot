...
- Coverage centres on the pure Telegram HTML machinery in `src/telegram-html.ts` (escaping, sanitizing, tag-aware splitting) — the code where a regression is silent because Telegram rejects a whole message on malformed markup — plus the prompt queue, steering, transport recovery, the background-work tools and their live progress messages, session notes, cron storage, the callback menus, and the Telegram transport. Sub-agent tools are tested against a fake extension API with a fake worker (`tests/subagent.test.ts`); nothing in `tests/` may start a real Pi session.

 ...
 * returns, so a tool that waits minutes (background_bash_wait, subagent_wait)
 * would hold the user's message for all of that time. SdkPiSession fires this
 * once the SDK has accepted a steered message; a waiting tool listens for its
 * own session and returns at once, and the message follows its result.
 *
